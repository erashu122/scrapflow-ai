...
public class BuyerService {
  private final BuyerProfileRepository buyers; private final BuyerDocumentRepository documents; private final DocumentStoragePort storage;
  public BuyerService(BuyerProfileRepository buyers, BuyerDocumentRepository documents, DocumentStoragePort storage) { this.buyers = buyers; this.documents = documents; this.storage = storage; }
  ...
  public BuyerDtos.BuyerDocumentResponse upload(String userId, BuyerDocumentType type, MultipartFile file) {
    BuyerProfile profile = profile(userId); validateFile(type, file); String extension = extension(file.getOriginalFilename()); String key = "buyers/" + profile.getId() + "/" + type.name().toLowerCase() + "/" + UUID.randomUUID() + extension;
    ...
    BuyerDocument document = new BuyerDocument(profile.getId(), type, key, file.getContentType(), file.getSize(), sha256(file)); documents.findByBuyerIdAndType(profile.getId(), type).ifPresent(existing -> documents.delete(existing)); return toResponse(documents.save(document));
  }
  ...
  private void validateFile(BuyerDocumentType type, MultipartFile file) { if (file == null || file.isEmpty()) throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "A document is required"); long max = type == BuyerDocumentType.COMPANY_LOGO ? 5_000_000L : 10_000_000L; if (file.getSize() > max) throw new ResponseStatusException(HttpStatus.PAYLOAD_TOO_LARGE, "Document exceeds the permitted size"); String contentType = file.getContentType(); boolean permitted = type == BuyerDocumentType.COMPANY_LOGO ? "image/png".equals(contentType) || "image/jpeg".equals(contentType) : "application/pdf".equals(contentType) || "image/png".equals(contentType) || "image/jpeg".equals(contentType); if (!permitted) throw new ResponseStatusException(HttpStatus.UNSUPPORTED_MEDIA_TYPE, "Unsupported document type"); }
  ...
  private String extension(String name) { if (name == null) return ""; int dot = name.lastIndexOf('.'); return dot < 0 ? "" : name.substring(dot).toLowerCase(); }
}
